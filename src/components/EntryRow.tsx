...
import { ordinalLabel } from '../utils/ordinal';
...
export function EntryRow({
  ...
  const title = showPersonName ? `${personName} · ${ordinalLabel(ordinal)}` : ordinalLabel(ordinal);
  return (
    <Pressable style={styles.row} onPress={() => router.push(`/date/${entry.id}`)}>
      ...
      <Text style={styles.date}>{formatShortDate(entry.occurredAt)}</Text>
...
const styles = StyleSheet.create({
  ...
  date: { fontFamily: fonts.sans, fontSize: 11, color: colors.inkFaint },
